...
function generateTable() {
	...
	descriptorSpan.innerText =
		'User activity over one year of time. Each column is one week, with older weeks to the left.';
...
async function fetchData(username, year, hue) {
	...
	for (let i = 0; i < 12; i++) {
		...
		for (let j = 0; j < pgns.length; j++) {
			...
			if (
				!annotations.hasOwnProperty('Black') ||
			...
			if (gameData[annotations.Date]) {
				...
				if (gameData[annotations.Date]['total'] > maxGamesPlayed)
					maxGamesPlayed = gameData[annotations.Date]['total'];
			} else {
	...
	for (const dateString of dateArray) {
		...
		if (gameData.hasOwnProperty(dateString)) {
			...
			const lightness = Math.floor(
				easeInPowerBounded(
					1 - (totalGames - threshold) / (maxGamesPlayed - threshold),
					lightnessFloor,
					lightnessCiel,
					5
				)
			...
			const text = `[${gameData[dateString]['win']}-${gameData[dateString]['loss']}-${gameData[dateString][
				'draw'
			]}] on ${datePretty}`;
...
// set the max attribute value for input form with id form-input-year
function setMaxYear() {
	const currentYear = new Date().getFullYear();
	const inputYear = document.getElementById('form-input-year');
